...
<script>
	...
	export default {
				...
				for(var i = 0; i < this.allItem.length; i++) {
					...
					if(this.currentSetion.id == this.allItem[i].id) {
						...
						if(index >= lastIndex) {
							...
							this.axios.get('https://novel.juhe.im/chapters/' + encodeURIComponent('http://vip.zhuishushenqi.com/chapter/' + i + '?cv=1529047156598')).then(setions => {
		...
		created() {
			...
			if(this.allItem != undefined){
				localStorage.setItem("allItem",JSON.stringify(this.allItem))
			...
			var i = this.currentSetion.id
			this.axios.get('https://novel.juhe.im/chapters/' + encodeURIComponent('https://vip.zhuishushenqi.com/chapter/' + i + '?cv=1529047156598')).then(setions => {
				localStorage.setItem('setions', JSON.stringify(setions))
				var links = JSON.parse(localStorage.getItem('setions')).data.chapter
				this.setions = links
